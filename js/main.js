document.addEventListener('DOMContentLoaded', function () {
  ...
  if (ocStage) {
    ...
    /* `duration` is how long a photo item holds before advancing to the next item
       in the same highlight; for a video it's just the setTimeout fallback (the
       video's own 'ended' event normally fires first and advances immediately).
       Caption is per-highlight, not per-item, on purpose -- every item in
